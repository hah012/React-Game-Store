...
const BestoftheYear = () => {
    ...
    const fetchGames = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.rawg.io/api/games?key=${API_KEY}&dates=2023-01-01,2023-12-31&ordering=-added&page=${currentPage}&page_size=${PAGE_SIZE}`
        );
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.rawg.io/api/games?key=${API_KEY}&dates=2023-01-01,2023-12-31&ordering=-added&page=${currentPage + 1}&page_size=${PAGE_SIZE}`
      );
      const data = await response.json();
      setGames((prevGames) => {
        const uniqueGames = new Map(prevGames.map(game => [game.id, game]));
  ...
  return (
    ...
    >
      ...
      {!initialLoading && (
        <div>
          <h2>Best Games of this Year</h2><br />
