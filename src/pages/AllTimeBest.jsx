...
const  AllTimeBest= () => {
    ...
    const fetchGames = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.rawg.io/api/games?key=${API_KEY}&page=${currentPage}&page_size=${PAGE_SIZE}`
        );
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.rawg.io/api/games?key=${API_KEY}&page=${currentPage + 1}&page_size=${PAGE_SIZE}`
      );
      const data = await response.json();
      setGames((prevGames) => {
        const uniqueGames = new Map(prevGames.map(game => [game.id, game]));
  ...
  return (
    ...
    >
      ...
      {!initialLoading && (
        <div>
          <h2>Top Games of all Time</h2><br />
